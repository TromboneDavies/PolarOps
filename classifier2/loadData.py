...
def vectorize(texts, vocab=None):
    ...
    Return values:
      - the array of vectorized texts: one row per text, and one column per
        feature. Each entry is a number, giving the number of occurrences of
        that feature in that text.
